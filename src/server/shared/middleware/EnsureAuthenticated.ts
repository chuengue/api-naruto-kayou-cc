...
export const ensureAuthenticated: RequestHandler = async (req, res, next) => {
    ...
    if (!authorization) {
        return sendErrorResponse(res, StatusCodes.UNAUTHORIZED, TLoginError(LoginErrors.UserNotAuthenticated));
    }
    const [type, token] = authorization.split(' ');

    if (type !== 'Bearer') {
        return sendErrorResponse(res, StatusCodes.UNAUTHORIZED, TLoginError(GenericErrors.InvalidAccessToken));
    }
    const jwtData = JWTService.verify(token);

    if (jwtData === EJWTErrors.SECRET_NOT_FOUND) {
        return sendErrorResponse(res, StatusCodes.INTERNAL_SERVER_ERROR, TLoginError(LoginErrors.VerifyErrorToken));
